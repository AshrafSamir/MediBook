...
const getAllBookings= async(req,res)=>{
    ...
    if(bookingsTemp.length){
        for (let i = 0; i < bookingsTemp.length; i++) {
            let user = await userModel.findOne({_id:bookingsTemp[i].patientId},{password:0,_v:0});
            let timeSlot = await timeSlotsModel.findOne({_id:bookingsTemp[i].timeSlotId})
            let doctorInfo = await doctorInfoModel.findOne({doctorId:timeSlot.doctorId});
            let doctor = await userModel.findOne({_id:timeSlot.doctorId},{password:0,_v:0});
            bookings.push({bookingInfo:bookingsTemp[i],user,timeSlot,doctor:{...doctor._doc,...doctorInfo._doc}});
        }
        if(bookings.length){
            res.json({bookings})
        }
...
const getBookingById= async(req,res)=>{
    ...
    let booking={};
    if(bookingTemp){
        let user = await userModel.findOne({_id:bookingTemp.patientId},{password:0,_v:0});
        let timeSlot = await timeSlotsModel.findOne({_id:bookingTemp.timeSlotId})
        let doctorInfo = await doctorInfoModel.findOne({doctorId:timeSlot.doctorId});
        let doctor = await userModel.findOne({_id:timeSlot.doctorId},{password:0,_v:0});
        booking={user , timeSlot , doctor:{...doctor._doc,...doctorInfo._doc}}
        res.json({booking})
    }
    else{
...
const getBookingData= async(req,res)=>{
    ...
    if(booking){
        ...
        if(bookingDataTemp.length){
            let user = await userModel.findOne({_id:booking.patientId},{password:0,_v:0});
            let timeSlot = await timeSlotsModel.findOne({_id:booking.timeSlotId})
            let doctorInfo = await doctorInfoModel.findOne({doctorId:timeSlot.doctorId});
            let doctor = await userModel.findOne({_id:timeSlot.doctorId},{password:0,_v:0});
            bookingData={bookingInfo:booking,data:bookingDataTemp,user,timeSlot,doctorInfo,doctor};
